...
EOF
wd_size="$(fdisk ld0d | sed -n -f /tmp/sed.$$)"
fdisk -i -a -0 -f -u -s 169/63/$(($wd_size - 63)) /dev/rld0d
...
for s in base etc comp games kern-GENERIC man misc modules tests text xbase xcomp xetc xfont xserver; do
  ( cd $r && tar --chroot -xpf /amd64/binary/sets/$s.tar.xz )
done
...
chroot $r /usr/sbin/installboot  /dev/rld0a /usr/mdec/bootxx_ffsv2
...
chroot $r /usr/pkg/bin/pkgin -y install sudo wget

# vagrant user
chroot $r useradd -d /home/vagrant -m -s /bin/sh -g staff -G wheel -p "$passhash" vagrant
